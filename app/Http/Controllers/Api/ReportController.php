...
class ReportController extends Controller
{
    ...
    public function generateCustom(Request $request): JsonResponse
    {
        ...
        $outreachSummary = $this->calculateOutreachSummary($user, $startDate, $endDate);
        $schemesEngagement = $this->getSchemesEngagementUpdate($user, $startDate, $endDate);
        $newLeads = $this->getNewLeads($user, $startDate, $endDate);
        $wonDeals = $this->getWonDeals($user, $startDate, $endDate);
        $lostDeals = $this->getLostDeals($user, $startDate, $endDate);
        $keyReminders = $this->getKeyReminders($user, $endDate);

        $report = Report::create([
            'user_id' => $user->id,
            'type' => 'custom',
            'report_date' => $validated['end_date'],
     ...
     */
    protected function calculateOutreachSummary($user, $startDate, $endDate): array
    {
     ...
     */
    protected function getSchemesEngagementUpdate($user, $startDate, $endDate): array
    {
        $leadIds = $this->getUserLeadIds($user);
     ...
     * Get new leads.
     */
    protected function getNewLeads($user, $startDate, $endDate): array
    {
        $leadIds = $this->getUserLeadIds($user);
     ...
     * Get won deals.
     */
    protected function getWonDeals($user, $startDate, $endDate): array
    {
        $leadIds = $this->getUserLeadIds($user);
     ...
     * Get lost deals.
     */
    protected function getLostDeals($user, $startDate, $endDate): array
    {
        $leadIds = $this->getUserLeadIds($user);
     ...
     */
    protected function getKeyReminders($user, $date): array
    {
        $leadIds = $this->getUserLeadIds($user);
     ...
     */
    protected function getUserLeadIds($user): \Illuminate\Support\Collection
    {
        if ($user->isAdmin()) {
